...
4. 나라장터 공고(`out/g2b/`)도 같은 기준으로 판정하되, R&D가 아닌 **용역 입찰**이므로 다음을 봅니다: 사업 내용이 X2R 역량에 맞는가, **brief 하단의 "입찰공고서 참가자격 발췌"에 적힌 입찰참가자격(업종 등록·직접생산확인증명·면허·실적·지역)을 company_profile.md 5절의 보유 자격과 대조**해 필수 자격을 보유하지 않으면 부적합·미확인이면 조건부, 기초금액이 2천만원~10억 범위인가, 입찰마감까지 3일 이상 남았는가. 제목이나 API 필드(업종제한 Y/N)만으로 판정하지 마세요.
